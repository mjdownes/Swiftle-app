                            ...
                            <div className="rounded-2xl w-full max-w-md flex justify-between border" style={{ backgroundColor: currentTheme.primary, color: currentTheme.secondary, borderColor: currentTheme.secondary }}>
                                ...
                                {options.map((range: string) => (
                                    <button key={range} onClick={() => { handleChange(range) }}
                                        className={twMerge('flex-1 py-2 px-2 my-2 mx-1 rounded-xl shadow-md border', selected === range ? "inset-shadow-sm/50" : "")} style={{ backgroundColor: selected === range ? currentTheme.background : currentTheme.primary, color: currentTheme.secondary, borderColor: currentTheme.secondary }}>
